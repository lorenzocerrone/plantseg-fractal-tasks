...
```console
# Install only plantseg_fractal_tasks:
python -m pip install -e .
# Install both plantseg_fractal_tasks and development dependencies (e.g. pytest):
...
```console
python src/plantseg_fractal_tasks/dev/create_manifest.py
git add src/plantseg_fractal_tasks/__FRACTAL_MANIFEST__.json
git commit -m'Update `__FRACTAL_MANIFEST__.json`'
git push origin main
```

5. If you add a new task, you should also add a new item to the `TASK_LIST`
list, in `src/plantseg_fractal_tasks/dev/task_list.py`. Here is an example:
```python
